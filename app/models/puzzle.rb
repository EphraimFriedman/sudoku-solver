      ...
      row.map!.with_index do |cell, index|
        ...
        if cell.length > 1
          ...
          # cell = checker(box, cell)
					...
					cell = cell - get_row_column_box(running_board, running_board.index(row), index )
...
def get_row_column_box(grid, row_index, cell_index)
	get_row(grid, row_index) + get_column(grid, cell_index) + get_box(grid, row_index, cell_index)

end

def get_row(grid, index)
	grid[index].reject {|num| num.is_a?(Array)}
end
