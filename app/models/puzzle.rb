    ...
    @board.map do |row|
      row.map!.with_index do |cell, index|
        ...
        if cell.length > 1
					...
					cell = cell - get_row_column_box(@board.index(row), index).reject {|num| num.is_a?(Array)}
...
## advanced logic for puzzles 5- 10
	...
	# # check the same against the colum and box posibilities
		# pos = cell - get_row_column_box(@board.index(row), index).select {|num| num.is_a?(Array)}.flatten
